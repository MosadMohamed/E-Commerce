...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('product_tag_pivot', function (Blueprint $table) {
            $table->integer('IDProductTag')->primary()->autoIncrement();
            $table->integer('IDProduct');
            $table->integer('IDTag');
            $table->boolean('ProductTagActive')->default(1);

            $table->dateTime('created_at')->default(DB::raw('CURRENT_TIMESTAMP'));
            $table->dateTime('updated_at')->default(DB::raw('CURRENT_TIMESTAMP'));

            $table->foreign('IDProduct')->references('IDProduct')->on('products')->onDelete('CASCADE')->onUpdate('CASCADE');
            $table->foreign('IDTag')->references('IDTag')->on('product_tags')->onDelete('CASCADE')->onUpdate('CASCADE');
